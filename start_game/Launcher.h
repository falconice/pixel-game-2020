...
public:
    ...
    {                                       // конструктор окна
        mode = sf::VideoMode({1920, 1080}); // VideoMode().getDesktopMode(); /// VideoMode(800, 400)
        title = "Pixel Game 2020";
        window = new RenderWindow(mode, title, Style::Close); // Игровое окно, на весь экран Style::Fullscreen

        view.reset(sf::Rect<float>({0, 0}, {1920, 1080}));
